...
for i in range(1,max_band+1):
    ...
    table = "cpc_prcp_{}".format(db_date)
    ...
    if tab_out == 0:
        ...
        os.system('gdal_translate -srcwin 0 0 360 360 -a_ullr 0 90 180 -90 -a_ullr 0 90 180 -90 {out}.tif {out}_east.tif'.format(out=name))
        os.system('gdal_translate -srcwin 360 0 360 360 -a_ullr -180 90 0 -90 {out}.tif {out}_west.tif'.format(out=name))
        ...
        cur.execute("SELECT AddRasterConstraints('cpc_prcp'::name, '{}'::name, 'rast'::name)".format(table))
        cur.execute("grant select on cpc_prcp.{} to public".format(table))
